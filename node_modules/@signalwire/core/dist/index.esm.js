...
import { fork as fork2, call as call2, take as take3, put as put2, all, cancelled as cancelled2 } from "@redux-saga/core/effects";
...
import { take as take2, cancelled } from "@redux-saga/core/effects";
...
function* initSessionSaga({
  ...
  const pubSubTask = yield fork2(pubSubSaga, {
    pubSubChannel,
  ...
  session.connect();
  yield take3(destroyAction.type);
  session.disconnect();
  yield take3(sessionDisconnectedAction.type);
  yield put2(pubSubChannel, sessionDisconnectedAction());
  pubSubTask.cancel();
...
function* reauthenticateWorker({
  ...
  try {
    if (session.reauthenticate) {
      ...
      yield put2(pubSubChannel, sessionConnectedAction());
    }
  } catch (error) {
    ...
    while (true) {
      ...
      switch (action.type) {
        ...
        case authExpiringAction.type: {
          yield put2(options.pubSubChannel, sessionExpiringAction());
          break;
        }
        ...
        case sessionReconnectingAction.type: {
          yield put2(options.pubSubChannel, sessionReconnectingAction());
          break;
        }
...
function* sessionAuthErrorSaga(options) {
  ...
  try {
    ...
    yield put2(pubSubChannel, sessionAuthErrorAction(error));
  } finally {
    if (yield cancelled2()) {
...
var rootSaga_default = (options) => {
  return function* root({
    ...
    while (true) {
      const action = yield take3([initAction.type, reauthAction.type]);
    ...
    return /* @__PURE__ */ new Map([
      [
        ["message"],
        {
          ...
          payloadTransform: (payload) => {
            const { channel: channel3, message } = payload.params;
...
import { fork as fork3, put as put3 } from "@redux-saga/core/effects";
...
function* memberUpdatedWorker({
  ...
  for (const key of updated) {
    ...
    yield put3(channels.pubSubChannel, {
      type,
