...
var voiceCallPromptWorker = function* (options) {
  ...
  while (run) {
    ...
    if (action.payload.result) {
      ...
      if (!typeToEmit) {
        getLogger8().info(`Unknown prompt result type: "${action.payload.result.type}"`);
        continue;
      }
      yield sagaEffects8.put(pubSubChannel, {
        type: typeToEmit,
...
var voiceCallDialWorker = function* (options) {
  ...
  if (action.payload.dial_state === "answered") {
    yield sagaEffects11.put(pubSubChannel, {
      type: "calling.call.state",
...
var voiceCallCollectWorker = function* (options) {
  getLogger14().trace("voiceCallCollectWorker started");
  const { channels, instance, initialState } = options;
  const { swEventChannel, pubSubChannel } = channels;
  const { controlId } = initialState;
  if (!controlId) {
  ...
  const done = () => run = false;
  while (run) {
    const action = yield sagaEffects14.take(swEventChannel, (action2) => {
      return action2.type === "calling.call.collect" && action2.payload.control_id === controlId;
    });
    const payloadWithTag = __spreadValues({
      tag: instance.tag
    }, action.payload);
    yield sagaEffects14.put(pubSubChannel, {
    ...
    if (action.payload.final === false) {
      ...
      if (action.payload.result) {
        ...
        if (!typeToEmit) {
          getLogger14().info(`Unknown collect result type: "${action.payload.result.type}"`);
          continue;
        }
        yield sagaEffects14.put(pubSubChannel, {
          type: typeToEmit,
  ...
  getLogger14().trace("voiceCallCollectWorker ended");
};
...
var voiceClientWorker = function* (options) {
  getLogger15().info("voiceClientWorker started");
  const { channels } = options;
  const { swEventChannel, pubSubChannel } = channels;
  function* worker(payload) {
    yield sagaEffects15.put(pubSubChannel, {
      type: `calling.call.state.${payload.call_id}`,
      payload
    });
  }
  while (true) {
    const action = yield sagaEffects15.take(swEventChannel, "calling.call.state");
    yield sagaEffects15.fork(worker, action.payload);
  }
  getLogger15().info("voiceClientWorker ended");
};
...
import {
  ...
  BaseComponent as BaseComponent9
} from "@signalwire/core";
var ENDED_STATES6 = [
  "error",
  "no_input",
...
var CallCollectAPI = class extends BaseComponent9 {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "calling");
    __publicField(this, "callId");
...
var callingCollectTriggerEvent = toLocalEvent3("calling.collect.trigger");
      ...
      [
        ...
        ],
        {
          ...
          afterCreateHook: (instance) => {
            ...
            const handler = (payload) => {
              instance.__sw_update_payload(toExternalJSON5(payload));
      ...
      [
        ["calling.connect.connected"],
        {
          ...
          payloadTransform: (payload) => {
            ...
            return toExternalJSON5(__spreadProps(__spreadValues({}, peerParams), { connect_state: "connected" }));
          }
        }
      ],
      [
        ...
        "calling.call.received",
        {
          ...
          afterCreateHook: (instance) => {
            ...
            const callStateHandler = (payload) => {
              instance.__sw_update_payload(toExternalJSON6(payload));
