...
var Session = class extends import_core6.BaseSession {
  constructor() {
    ...
    __publicField(this, "CloseEventConstructor", import_core6.CloseEvent);
    __publicField(this, "agent", "@signalwire/nodejs/realtime-api/3.9.0");
  }
};
...
var voiceCallPromptWorker = function* (options) {
  ...
  while (run) {
    ...
    if (action.payload.result) {
      ...
      if (!typeToEmit) {
        (0, import_core29.getLogger)().info(`Unknown prompt result type: "${action.payload.result.type}"`);
        continue;
      }
      yield import_core29.sagaEffects.put(pubSubChannel, {
        type: typeToEmit,
...
var voiceCallDialWorker = function* (options) {
  ...
  if (action.payload.dial_state === "answered") {
    yield import_core32.sagaEffects.put(pubSubChannel, {
      type: "calling.call.state",
...
var voiceCallCollectWorker = function* (options) {
  (0, import_core35.getLogger)().trace("voiceCallCollectWorker started");
  const { channels, instance, initialState } = options;
  const { swEventChannel, pubSubChannel } = channels;
  const { controlId } = initialState;
  if (!controlId) {
  ...
  const done = () => run = false;
  while (run) {
    const action = yield import_core35.sagaEffects.take(swEventChannel, (action2) => {
      return action2.type === "calling.call.collect" && action2.payload.control_id === controlId;
    });
    const payloadWithTag = __spreadValues({
      tag: instance.tag
    }, action.payload);
    yield import_core35.sagaEffects.put(pubSubChannel, {
    ...
    if (action.payload.final === false) {
      ...
      if (action.payload.result) {
        ...
        if (!typeToEmit) {
          (0, import_core35.getLogger)().info(`Unknown collect result type: "${action.payload.result.type}"`);
          continue;
        }
        yield import_core35.sagaEffects.put(pubSubChannel, {
          type: typeToEmit,
  ...
  (0, import_core35.getLogger)().trace("voiceCallCollectWorker ended");
};

// src/voice/workers/voiceClientWorker.ts
var import_core36 = require("@signalwire/core");
var voiceClientWorker = function* (options) {
  (0, import_core36.getLogger)().info("voiceClientWorker started");
  const { channels } = options;
  const { swEventChannel, pubSubChannel } = channels;
  function* worker(payload) {
    yield import_core36.sagaEffects.put(pubSubChannel, {
      type: `calling.call.state.${payload.call_id}`,
      payload
    });
  }
  while (true) {
    const action = yield import_core36.sagaEffects.take(swEventChannel, "calling.call.state");
    yield import_core36.sagaEffects.fork(worker, action.payload);
  }
  (0, import_core36.getLogger)().info("voiceClientWorker ended");
};

// src/voice/CallPlayback.ts
var import_core37 = require("@signalwire/core");
var ENDED_STATES = ["finished", "error"];
var CallPlaybackAPI = class extends import_core37.BaseComponent {
  constructor() {
    super(...arguments);
...
var CallRecordingAPI = class extends import_core38.BaseComponent {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "calling");
    __publicField(this, "callId");
...
var createCallRecordingObject = (params) => {
  const record = (0, import_core38.connect)({
    store: params.store,
...
var CallPromptAPI = class extends import_core39.BaseComponent {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "calling");
    __publicField(this, "callId");
...
var ENDED_STATES4 = ["finished"];
var CallTapAPI = class extends import_core40.BaseComponent {
  constructor() {
    super(...arguments);
    __publicField(this, "callId");
    __publicField(this, "nodeId");
...
var createCallTapObject = (params) => {
  const tap = (0, import_core40.connect)({
    store: params.store,
...
var ENDED_STATES5 = ["finished", "error"];
var CallDetectAPI = class extends import_core41.BaseComponent {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "calling");
    __publicField(this, "callId");
  ...
  ended() {
    var _a, _b;
    if (ENDED_STATES5.includes((_b = (_a = this.detect) == null ? void 0 : _a.params) == null ? void 0 : _b.event)) {
      return Promise.resolve(this);
    }
    return new Promise((resolve) => {
      this._attachListeners(this.controlId);
...
var createCallDetectObject = (params) => {
  const detect = (0, import_core41.connect)({
    store: params.store,
...
var CallCollectAPI = class extends import_core42.BaseComponent {
  constructor() {
    super(...arguments);
    __publicField(this, "_eventsPrefix", "calling");
    __publicField(this, "callId");
  ...
  ended() {
    var _a;
    if (ENDED_STATES6.includes((_a = this.result) == null ? void 0 : _a.type)) {
      return Promise.resolve(this);
    }
    return new Promise((resolve) => {
      this._attachListeners(this.controlId);
...
var createCallCollectObject = (params) => {
  const collect = (0, import_core42.connect)({
    store: params.store,
...
// src/voice/Call.ts
var callingPlaybackTriggerEvent = (0, import_core43.toLocalEvent)("calling.playback.trigger");
var callingRecordTriggerEvent = (0, import_core43.toLocalEvent)("calling.recording.trigger");
var callingPromptTriggerEvent = (0, import_core43.toLocalEvent)("calling.prompt.trigger");
var callingTapTriggerEvent = (0, import_core43.toLocalEvent)("calling.tap.trigger");
var callingDetectTriggerEvent = (0, import_core43.toLocalEvent)("calling.detect.trigger");
var callingCollectTriggerEvent = (0, import_core43.toLocalEvent)("calling.collect.trigger");
var CallConsumer = class extends AutoApplyTransformsConsumer {
  constructor(options) {
    ...
    __publicField(this, "connectState");
    ...
    return /* @__PURE__ */ new Map([
      [
        ...
        ],
        {
          ...
          afterCreateHook: (instance) => {
            const eventName = `call.play.${instance.controlId}`;
            const handler = (payload) => {
              instance.__sw_update_payload((0, import_core43.toExternalJSON)(payload));
          ...
          payloadTransform: (payload) => {
            return (0, import_core43.toExternalJSON)(payload);
          }
        }
      ],
      [
      ...
      [
        ["calling.connect.connected"],
        {
          ...
          payloadTransform: (payload) => {
            ...
            return (0, import_core43.toExternalJSON)(__spreadProps(__spreadValues({}, peerParams), { connect_state: "connected" }));
          }
        }
      ],
      [
  ...
  dial(params) {
    return new Promise((resolve, reject) => {
      ...
      const _a = params, { devices } = _a, rest = __objRest(_a, ["devices"]);
      const executeParams = __spreadValues({
        tag: this.__uuid
      }, (0, import_core43.toSnakeCaseKeys)(rest));
  ...
  play(params) {
    return new Promise((resolve, reject) => {
      ...
      const controlId = (0, import_core43.uuid)();
  ...
  record(params) {
    return new Promise((resolve, reject) => {
      ...
      const record = (0, import_core43.toSnakeCaseKeys)(params);
      this.execute({
  ...
  prompt(params) {
    return new Promise((resolve, reject) => {
      ...
      const { initial_timeout, digits, speech } = (0, import_core43.toSnakeCaseKeys)(params);
      const collect = {
        initial_timeout,
        digits,
        speech
  ...
  connect(params) {
    return new Promise((resolve, reject) => {
      ...
      const rejectHandler = (payload) => {
        ...
        reject((0, import_core43.toExternalJSON)(payload));
      };
    ...
    return new Promise((resolve, reject) => {
      ...
      this.execute({
        ...
        params: {
          ...
          detect: {
            type,
            params: (0, import_core43.toSnakeCaseKeys)(rest)
          }
        }
      }).then(() => {
        const startEvent = {
  ...
  collect(params) {
    return new Promise((resolve, reject) => {
      ...
      const resolveHandler = (callCollect) => {
        resolve(callCollect);
      };
      ...
      const {
        ...
        send_start_of_input,
        start_input_timers
      } = (0, import_core43.toSnakeCaseKeys)(params);
      this.execute({
...
var CallAPI = (0, import_core43.extendComponent)(CallConsumer, {});
var createCallObject = (params) => {
  const call = (0, import_core43.connect)({
    store: params.store,
    Component: CallAPI
  })(params);
  return call;
      ...
      [
        "calling.call.received",
        {
          ...
          afterCreateHook: (instance) => {
            ...
            const callStateHandler = (payload) => {
              instance.__sw_update_payload((0, import_core44.toExternalJSON)(payload));
          ...
          payloadTransform: (payload) => {
            return (0, import_core44.toExternalJSON)(payload);
          }
        }
      ]
    ]);
...
var createVoiceObject = (params) => {
  const voice = (0, import_core44.connect)({
    store: params.store,
    Component: VoiceAPI
  })(params);
  return voice;
